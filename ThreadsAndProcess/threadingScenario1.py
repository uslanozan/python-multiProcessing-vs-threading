...
download_directory = "Images/ImageFromThreading"
...
if __name__ == "__main__":
    print("IMAGE DOWNLOADING WITH THREADING (SCENARIO 1)")
    threads = []
    times = [0] * len(urls)
    cpu_usages = [0] * len(urls)
    memory_usages = [0] * len(urls)
    counter = 1

    # Create a thread for each URL and start downloading
    for url in urls:
        thread = threading.Thread(target=download_file, args=(url, counter, download_directory, times, cpu_usages, memory_usages))
        # It creates a new thread. It starts a process with the target function download_file
    ...
    for i, t in enumerate(times):
        print(f"Download time for Image {i + 1}: {t:.2f} seconds, CPU usage: {cpu_usages[i]:.2f}%, Memory usage: {memory_usages[i]:.2f} MB")
